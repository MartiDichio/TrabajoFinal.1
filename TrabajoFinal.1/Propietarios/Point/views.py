...
def avance_list(request):
    ...
    return render(request, 'Point/avance_list.html', {'avances': avances}) ##aca tenemos vista del listado

def avance_detail(request, pk):
    avance = get_object_or_404(Avance, pk=pk)
    return render(request, 'Point/avance_detail.html', {'avance': avance})  ##aca vista de los detalles
...
def avance_create(request):
    ...
    return render(request, 'point/avance_form.html')
